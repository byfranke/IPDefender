...
usage() {
  ...
  echo "  $0 --ban <IP> [reason]    Ban IP with threat intelligence"
  echo "  $0 --check <IP>           Analyze IP reputation"
  echo "  $0 --unban <IP>           Remove IP ban"
  echo "  $0 --unban-all            Remove all bans"
  echo "  $0 --list                 Show active bans"
  echo "  $0 --api-abuseipdb <KEY>  Configure AbuseIPDB API key"
